...
- 2026-06-17 01:00 CT: GitHub Actions workflows should use Node 24-native action pins for Python and SAM setup (`actions/setup-python@v6`, `aws-actions/setup-sam@v3`). Do not use `FORCE_JAVASCRIPT_ACTIONS_TO_NODE24` as the steady-state fix because GitHub still annotates actions that declare Node 20 even when forced.
